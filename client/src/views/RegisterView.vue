    ...
    <div @submit.prevent="handleRegister" class="w-80 bg-white rounded-xl shadow-md">
        <form class="flex flex-col text-center px-8 py-3">
            ...
            <button :disabled="authStore.isLoading" type="submit"
                class="w-full bg-blue-600 text-white text-md font-semibold rounded-md p-3 mb-2">
                Signup
            </button>
            <p class="text-sm">
                You already have an account?
                <router-link :to="{ name: 'login' }" class="underline text-blue-400">Login</router-link>
            </p>
        </form>
    </div>
</template>
